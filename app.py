...
def ai_analysis_text(p):
    if p.get("decision") == "BET":
        return f"{p['pick']}은 시장 암시확률 {p['market_probability']}% 대비 AI 예상승률 {p['ai_probability']}%로 Edge {p['ai_edge']}%가 있습니다. EV {p['ev']}%, Kelly {p['kelly']}%로 양호합니다."
    if p.get("decision") == "WATCH":
        return f"{p['pick']}은 조건은 나쁘지 않지만 강한 확신은 부족합니다. AI Edge {p['ai_edge']}%, EV {p['ev']}% 수준이라 관찰이 적절합니다."
    return f"{p['pick']}은 현재 No Bet입니다. AI Edge {p['ai_edge']}%, EV {p['ev']}%, 위험도 {p['risk']} 기준에서 추천 근거가 충분하지 않습니다."
...
def build_summary(picks, combos, no_bet):
    if not picks:
        return {
            ...
            "no_bet": True,
            "message": "10~120분 안에 분석 가능한 경기가 없습니다.",
    ...
    excluded = [
        {
            ...
            "risk": p["risk"],
            "decision": p["decision"],
            "reason": "AI 기준 미달 또는 No Bet",
        }
        for p in picks
        if p["decision"] == "NO_BET"
    ]
...
def health():
    return jsonify({
        "status": "ok",
        "version": "V7-ai-decision-engine",
        "odds_api_key": bool(ODDS_API_KEY),
        "time_filter": {"min_minutes": MIN_START_MINUTES, "max_minutes": MAX_START_MINUTES},
        "bmbets": "disabled_on_render_free_memory_limit",
    })
